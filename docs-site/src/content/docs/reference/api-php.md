...
description: "PHP API for Kreuzberg via ext-php-rs: extraction functions, configuration, and result objects."
---
...
Batch-level fields (`$maxConcurrentExtractions`, `$useCache`, `$securityLimits`) cannot be overridden per file. See [Configuration Reference](/reference/configuration/#fileextractionconfig) for details.
...
Kreuzberg integrates with LLMs via the `liter-llm` crate for structured extraction and VLM-based OCR. The PHP binding exposes typed `LlmConfig` and `StructuredExtractionConfig` classes. See the [LLM Integration Guide](/guides/llm-integration/) for full details.
