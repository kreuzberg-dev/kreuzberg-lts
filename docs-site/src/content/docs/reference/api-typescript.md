...
description: "TypeScript/Node.js API for Kreuzberg via NAPI-RS: async extraction functions, configuration, and result types."
---

Complete reference for the Kreuzberg **native TypeScript/Node.js** API (`@kreuzberg/node`).

:::note[WASM Alternative]
This reference covers **native bindings** (`@kreuzberg/node`) for Node.js, Bun, and Deno.

For browser and edge environments, see the [WASM API Reference](/reference/api-wasm/) (`@kreuzberg/wasm`).
:::
...
See [Configuration Reference](/reference/configuration/#fileextractionconfig) for full details on merge semantics and excluded batch-level fields.
