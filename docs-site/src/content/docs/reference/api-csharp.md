...
description: "C# API for Kreuzberg via P/Invoke over the native FFI: extraction methods, configuration, and result types."
---
...
```xml title=".csproj"
<ItemGroup>
    <PackageReference Include="Kreuzberg" Version="4.10.2" />
</ItemGroup>
```
...
Kreuzberg integrates with LLMs via the `liter-llm` crate for structured extraction and VLM-based OCR. The C# binding passes LLM configuration through the FFI/P/Invoke layer as JSON. See the [LLM Integration Guide](/guides/llm-integration/) for full details.
...
- [Data Types Reference](/reference/types/)
- [Error Handling Guide](/reference/errors/)
