...
description: "Elixir API for Kreuzberg via a Rustler NIF: extraction functions, configuration, and result structs."
---
...
Kreuzberg integrates with LLMs via the `liter-llm` crate for structured extraction and VLM-based OCR. The Elixir binding passes LLM configuration as map options through the Rustler NIF layer. See the [LLM Integration Guide](/guides/llm-integration/) for full details.
...
- [Error Handling Reference](/reference/errors/)
