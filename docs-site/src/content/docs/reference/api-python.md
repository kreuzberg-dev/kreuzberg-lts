...
description: "Python API for Kreuzberg via PyO3: sync and async extraction functions, configuration, and result dataclasses."
---
...
- `content_filter` (`ContentFilterConfig | None`) <span class="version-badge">v4.8.0</span>: Header, footer, watermark, and repeating-text filtering. Default: `None` (each extractor uses its built-in defaults). See [ContentFilterConfig](/reference/configuration/#contentfilterconfig).
...
- `content_filter` (ContentFilterConfig | None) <span class="version-badge">v4.8.0</span>: Override header/footer/watermark/repeating-text filtering. See [ContentFilterConfig](/reference/configuration/#contentfilterconfig).
...
See [Configuration Reference](/reference/configuration/#fileextractionconfig) for full details on merge semantics.
...
`register_document_extractor()` is not exposed to Python. Extractor *implementation and registration* must be done in Rust. See the [Creating Plugins Guide](/guides/plugins/#document-extractors) for the Rust API.
:::
...
All errors inherit from **`KreuzbergError`**. See [Error Handling Reference](/reference/errors/) for complete documentation.
...
:::caution[Different integer space from `ErrorCode`]
...
See [Error Handling Reference](/reference/errors/) for the complete error documentation.
...
:::caution[Deprecated since v4.2.0]
...
Kreuzberg integrates with LLMs via the `liter-llm` crate for structured extraction and VLM-based OCR. See the [LLM Integration Guide](/guides/llm-integration/) for full details.
