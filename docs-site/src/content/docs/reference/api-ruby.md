...
description: "Ruby API for Kreuzberg via Magnus: extraction methods, configuration, and result objects."
---
...
Kreuzberg integrates with LLMs via the `liter-llm` crate for structured extraction and VLM-based OCR. The Ruby binding passes LLM configuration as hash options through the native extension. See the [LLM Integration Guide](/guides/llm-integration/) for full details.
