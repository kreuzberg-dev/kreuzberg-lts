...
description: "Go API for Kreuzberg via cgo over the native FFI: the Extractor type, configuration, and result structs."
---
...
curl -LO https://github.com/kreuzberg-dev/kreuzberg-lts/releases/download/v4.10.4/go-ffi-linux-x86_64.tar.gz
...
### BatchExtractFilesWithConfigs <span class="version-badge">v4.10.4</span>
...
### BatchExtractFilesWithConfigsSync <span class="version-badge">v4.10.4</span>
...
### BatchExtractBytesWithConfigs / BatchExtractBytesWithConfigsSync <span class="version-badge">v4.10.4</span>

Batch extract multiple byte arrays with per-file configuration overrides. Async and sync variants follow the same pattern.

---

### FileExtractionConfig <span class="version-badge">v4.10.4</span>
...
Batch-level fields (`MaxConcurrentExtractions`, `UseCache`, `Acceleration`, `SecurityLimits`) cannot be overridden per file. See [Configuration Reference](/reference/configuration/#fileextractionconfig) for details.
...
- `string`: Version string (for example, "4.10.4")
...
type PdfConfig struct {
	AllowSingleColumnTables *bool       // <span class="version-badge">v4.10.4</span> Allow extraction of single-column tables
...
### ConcurrencyConfig <span class="version-badge">v4.10.4</span>
...
### LayoutDetectionConfig <span class="version-badge">v4.10.4</span>
...
type PaddleOcrConfig struct {
	...
	Language             string   // Language code
	ModelTier            string   // (v4.10.4) Model tier: "mobile" (default, ~21MB total, fast) or "server" (~172MB, best with GPU)
	Padding              *int     // (v4.10.4) Padding in pixels (0-100) around image before detection. Default: 10
...
Kreuzberg integrates with LLMs via the `liter-llm` crate for structured extraction and VLM-based OCR. The Go binding passes LLM configuration as JSON through the FFI layer. See the [LLM Integration Guide](/guides/llm-integration/) for full details.
...
For configuration details including API keys, model selection, and provider setup, see the [LLM Integration Guide](/guides/llm-integration/).
...
For configuration details, see the [Code Intelligence Guide](/guides/code-intelligence/).
