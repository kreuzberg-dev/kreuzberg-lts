...
description: "R API for Kreuzberg via an extendr native extension: extraction functions, configuration, and result lists."
---
...
Kreuzberg integrates with LLMs via the `liter-llm` crate for structured extraction and VLM-based OCR. The R binding passes LLM configuration as list options through the `extendr` FFI layer. See the [LLM Integration Guide](/guides/llm-integration/) for full details.
