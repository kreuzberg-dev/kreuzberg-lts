...
static RE_UK: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"\b[A-Z]{1,2}[0-9][0-9A-Z]?\s?[0-9][A-Z]{2}\b").expect("uk postcode regex compiles")
});
...
static RE_CA: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"\b[A-Z][0-9][A-Z]\s?[0-9][A-Z][0-9]\b").expect("ca postal regex compiles")
});
