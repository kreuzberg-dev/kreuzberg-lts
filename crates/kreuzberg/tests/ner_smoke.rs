...
//! upstream `gline-rs` crate pins `ort = "=2.0.0-rc.9"` while kreuzberg uses
...
use kreuzberg::text::ner::{default_model_name, download_model};
...
async fn detects_person_org_location_in_canonical_sentence() {
    ...
    use kreuzberg::text::ner::gline::GlineBackend;
    ...
    match result {
        ...
        Err(kreuzberg::KreuzbergError::MissingDependency(msg)) => {
...
async fn custom_labels_route_through_backend() {
    use kreuzberg::core::config::ner::NerConfig;
    use kreuzberg::text::ner::NerBackend;
    use kreuzberg::text::ner::gline::GlineBackend;
    use kreuzberg::types::entity::EntityCategory;
    ...
    let backend = match GlineBackend::new(None) {
        Ok(b) => b,
        Err(kreuzberg::KreuzbergError::MissingDependency(_)) | Err(kreuzberg::KreuzbergError::Plugin { .. }) => {
