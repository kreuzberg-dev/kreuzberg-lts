...
This is the long-term-support line of the document-extraction library, so it processes **hostile input documents**. In scope: decompression bombs, entity expansion, deeply nested structures, unbounded content growth, path traversal from archive members, and panics in any extractor. Because this is an LTS line, report issues here even when the same fix has already landed upstream.
