...
pub use ocr_backend::{clear_ocr_backends, list_ocr_backends, register_ocr_backend, unregister_ocr_backend};
...
use kreuzberg::plugins::{
    clear_extractors as kz_clear_extractors, clear_validators as kz_clear_validators,
    list_extractors as kz_list_extractors, list_post_processors as kz_list_post_processors,
...
pub fn unregister_validator(name: String) -> Result<(), Error> {
    kz_unregister_validator(&name).map_err(crate::error_handling::kreuzberg_error)
}
