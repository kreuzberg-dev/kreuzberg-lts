 ...
 *
 * Token-based sizing uses HuggingFace tokenizers loaded at runtime, or a tokenizer backend you
 * register yourself. Any tokenizer available on HuggingFace Hub can be used, including
 * OpenAI-compatible tokenizers (e.g., `Xenova/gpt-4o`, `Xenova/cl100k_base`). To size chunks with
 * your own tokenizer instead (llama.cpp/GGUF vocabularies, SentencePiece models, custom vocabs),
 * register a `TokenizerBackend` with `register_tokenizer_backend` and set `model` to the registered
 * name.
 */
